...
func TestActivationStore_createsSelfActivateRequest(t *testing.T) {
	...
	result, err := store.Activate(context.Background(), "user-1", "/roleDefs/111", "/subscriptions/abc", "Deploy", 2*time.Hour)
	if err != nil {
...
func TestActivationStore_mapsPermissionDenied(t *testing.T) {
	...
	_, err := store.Activate(context.Background(), "user-1", "/roleDefs/111", "/subscriptions/abc", "Deploy", 2*time.Hour)
